...
public class ConverterInheritanceTestCase {
  ...
  /**
   * Note: mapping by raw type doesn't work, because there's no way to know at runtime that an
   * ArrayList has been parameterized by 'Person'; it just looks like an ArrayList of 'Object'
   */
  ...
  public void testConverterInheritanceGenerics() {
    ...
    Type<Collection<Person>> personCollectionType =
        new TypeBuilder<Collection<Person>>() {}.build();
    ...
    PersonGroup group =
        factory.getMapperFacade().map(people, personCollectionType, personGroupType);
    Assert.assertNotNull(group);
    Assert.assertTrue(people.containsAll(group.getPeople()));
    Assert.assertTrue(group.getPeople().containsAll(people));
  }
  ...
  public static class ListConverter extends CustomConverter<Collection<Person>, PersonGroup> {
    ...
    public PersonGroup convert(
        Collection<Person> source,
        Type<? extends PersonGroup> destinationType,
