...
public abstract class MappingUtil {
  ...
  /**
   * Set this system property to true (typically during a build) to make sure no unit tests are
   * inadvertently using debug mode (EclipseJdtCompilerStrategy); we have a special TestSuite which
   * re-runs all unit tests using EclipseJdtCompilerStrategy, so if any unit tests are explicitly
   * using it, they will prevent testing with JavassistCompilerStrategy
   */
   ...
   * @param debugMode if true, EclipseJdt will be used for the compiler strategy (for step-debugging
   *     in IDEs), and class and source files will be written to disk.
   */
