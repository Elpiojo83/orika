...
public class GenericCollectionsTestCase {
  ...
  /**
   * This test uses the older Class-based mapping method, which is unable to determine the proper
   * type hierarchy from the raw types
   */
  @Test
  public void testParameterizedCollection_rawTypes() {
    ...
    ClassMapBuilder<?, ?> builder =
        factory.classMap(TaskLayer1.class, TaskLayer2.class).byDefault();
    ...
    factory
        .getConverterFactory()
        .registerConverter(new PassThroughConverter(Employee.class, Person.class));
  ...
  /**
   * This test attempts the same mapping using the newer type-based methods which allow passing in
   * the exact runtime types.
   */
  @Test
  public void testParameterizedCollection_genericTypes() {
    ...
    factory.registerClassMap(factory.classMap(sourceType, targetType).byDefault());
