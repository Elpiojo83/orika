...
public class SortedCollectionTestCase {
  ...
  public void simpleOrdering() {
    final OrderingRelation[][] relations =
        new OrderingRelation[][] {
          {
            OrderingRelation.EQUAL,
            OrderingRelation.AFTER,
            OrderingRelation.UNDEFINED,
            OrderingRelation.UNDEFINED
          },
          {
    ...
    Ordering<Integer> testOrdering =
        new Ordering<Integer>() {
          @Override
    ...
    SortedCollection<Integer> sorted =
        new SortedCollection<Integer>(Arrays.asList(3, 1, 0, 2), testOrdering);
    Assert.assertEquals(Arrays.asList(0, 1, 2, 3), new ArrayList<Integer>(sorted));
  }
  ...
  public void testOrdering2() {
    ...
    for (MapperKey key : queue) {
      for (MapperKey seenKey : seen) {
        if ((seenKey.getAType().isAssignableFrom(key.getAType())
                && seenKey.getBType().isAssignableFrom(key.getBType()))
            || (seenKey.getAType().isAssignableFrom(key.getBType())
                && seenKey.getBType().isAssignableFrom(key.getAType()))) {
