...
public class PropertyResolverExtensibilityTestCase {
  ...
  /**
   * This test demonstrates how you might implement a custom property resolver which provides a
   * proprietary definition of properties
   */
  @Test
  public void testAdHocResolution_integration_customResolverUsingDeclarativeProperties() {
    ...
    factory
        .classMap(Element.class, Person.class)
    ...
    Assert.assertEquals(
        ((Element) person.getAttribute("employment")).getAttribute("jobTitle"), result.jobTitle);
    ...
    Assert.assertEquals(
        ((Element) person.getAttribute("employment")).getAttribute("jobTitle"),
        ((Element) mapBack.getAttribute("employment")).getAttribute("jobTitle"));
  }

  /**
   * For my next trick, this is a custom Property Resolver which automatically attempts a dynamic
   * definition of type Element (after normal resolution has failed)
   */
