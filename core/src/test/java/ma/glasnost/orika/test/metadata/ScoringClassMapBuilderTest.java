...
public class ScoringClassMapBuilderTest {
  ...
  public void testClassMapBuilderExtension() {
    ...
    ClassMap<Source, Destination> map =
        factory.classMap(Source.class, Destination.class).byDefault().toClassMap();
    ...
    Map<String, List<List<String>>> tests =
        new HashMap<String, List<List<String>>>() {
...

          {
            ...
            put(
                "aVeryLongWord.name.first",
                asList(asList("a", "very", "long", "word"), asList("name"), asList("first")));
          }
        };

    Method splitIntoWords =
        ScoringClassMapBuilder.FieldMatchScore.class.getDeclaredMethod(
            "splitIntoLowerCaseWords", String.class);
