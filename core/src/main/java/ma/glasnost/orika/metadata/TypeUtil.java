...
abstract class TypeUtil {
  ...
  private static final Set<Type<?>> IGNORED_TYPES =
      new HashSet<Type<?>>(
          Arrays.asList(
  ...
  private static final Map<String, Class<?>> PRIMITIVES_CLASSES =
  ...
  static java.lang.reflect.Type[] resolveActualTypeArguments(
      final ParameterizedType type, final Type<?> reference) {

    return resolveActualTypeArguments(
  ...
  /**
   * Resolves the array of provided actual type arguments using the actual types of the reference
   * type and the type variables list for comparison.
   *
   * @param vars
   * @param typeArguments
   * @param reference
  ...
  static java.lang.reflect.Type[] resolveActualTypeArguments(
    ...
    java.lang.reflect.Type[] defaultTypeArguments =
        new java.lang.reflect.Type[typeArguments.length];
      ...
      for (int i = 0, len = actualTypeArguments.length; i < len; ++i) {
        ...
        if (typeArg instanceof ParameterizedType || typeArg instanceof Class) {
          ...
          while (!TypeFactory.TYPE_OF_OBJECT.equals(targetReference)) {
            ...
            if (typeFromReference != null) break;
            targetReference =
                TypeFactory.valueOf(targetReference.getRawType().getGenericSuperclass());
          }
          if (typeFromReference != null && typeArg.equals(var)) {
            if (actualTypeArguments[i] == null
                || (actualTypeArguments[i] instanceof Type
                    && ((Type<?>) actualTypeArguments[i]).isAssignableFrom(typeFromReference))) {
            ...
            if (defaultTypeArguments[i] == null
                || defaultTypeArguments[i].equals(TypeFactory.TYPE_OF_OBJECT)) {
  ...
  /**
   * Attempts to determine the more specific type out of the two provided types.<br>
   *
   * @param type0
   * @param type1
   */
  ...
  /**
   * Attempts to determine the more specific type out of the two provided types.<br>
   * Allows a provided list of types to ignore (which are basically considered the same as Object's
   * type in terms of their specificity); this is to allow ignoring types that are not a useful part
   * of the hierarchy comparison.
   *
   * @param type0
   * @param type1
   * @param ignoredTypes
   * @return
   */
  private static Type<?> getMostSpecificType(
      final Type<?> type0, final Type<?> type1, final Set<Type<?>> ignoredTypes) {
    if (type1 == null && type0 == null) {
      ...
      throw new IllegalArgumentException(
          "types " + type0 + " and " + type1 + " are not comparable");
    }
  }

  /**
   * Converts the provided list of actual type arguments to their equivalent Type representation.
   *
   * @param rawType
   * @param actualTypeArguments
   * @return
   */
  static Type<?>[] convertTypeArguments(
      final Class<?> rawType,
  ...
  /**
   * Use the ancestry of a given raw type in order to determine the most specific type parameters
   * possible from the known bounds.
   *
   * @param rawType
   * @return
   */
  private static Type<?>[] convertTypeArgumentsFromAncestry(
      final Class<?> rawType, final Set<java.lang.reflect.Type> bounds) {
   ...
   */
  static Type<?> parseTypeDescriptor(final String typeDescriptor)
      throws InvalidTypeDescriptorException {
  ...
  /**
   * Attempt to load the class specified by the given name, using the provided class loader.<br>
   * Additional attempts are made for classes not found which have no package declaration, under the
   * assumption that the class may be in the 'java.lang' package, or 'java.util' package (in that
   * order).
   *
   * @param name
   * @param cl
  ...
  private static Class<?> loadClass(final String name, final ClassLoader cl) {
    ...
    try {
      ...
      if (!name.contains(".")) {
        ...
        while (lastDot > 0) {
          modifiedName =
              modifiedName.substring(0, lastDot) + "$" + modifiedName.substring(lastDot + 1);
          try {
